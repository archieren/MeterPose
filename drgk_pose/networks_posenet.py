...
def conv2d_fixed_padding(inputs, filters, kernel_size, strides, data_format):
    ...
    return tf.compat.v1.layers.conv2d(
        inputs=inputs, filters=filters, kernel_size=kernel_size, strides=strides, padding=('SAME' if strides == 1 else 'VALID'), use_bias=False, kernel_initializer=tf.compat.v1.variance_scaling_initializer(), data_format=data_format)


def heatmap_gen(inputs, filters, kernel_size, strides, data_format, training, num_gen_layers=3, padding='SAME', use_bias=False):
    for _ in range(num_gen_layers):
        inputs = tf.compat.v1.layers.conv2d_transpose(
            inputs=inputs, filters=filters, kernel_size=kernel_size, strides=strides, padding=padding, data_format=data_format, use_bias=use_bias)
        inputs = batch_norm(
            inputs=inputs, training=training, data_format=data_format)
...
class Model(object):
    ...
    def __init__(self,
                 resnet_size,
                 bottleneck,
                 num_classes,  # 我这儿另外用途，作为num_points的别名算了
                 num_filters,
                 kernel_size,
                 conv_stride,
                 first_pool_size,
    ...
    def __call__(self, inputs, training):
        ...
        with self._model_variable_scope():
            ...
            for i, num_blocks in enumerate(self.block_sizes):
                ...
                inputs = block_layer(inputs=inputs, filters=num_filters, bottleneck=self.bottleneck, block_fn=self.block_fn, blocks=num_blocks,
                                     strides=self.block_strides[i], training=training, name='block_layer{}'.format(i + 1), data_format=self.data_format)
            ...
            # 添加生成部分！！
            inputs = heatmap_gen(inputs=inputs, filters=256, kernel_size=4, strides=2, num_gen_layers=self.num_gen_layers, data_format=self.data_format, training=training
                                 )
            inputs = tf.compat.v1.layers.conv2d(
                inputs=inputs, filters=self.num_classes, kernel_size=1, strides=1, padding='SAME', use_bias=False, kernel_initializer=tf.compat.v1.variance_scaling_initializer(), data_format=self.data_format)

            if self.data_format == 'channels_first':
                # 从内部使用的NCHW改成外部用的NHWC.
                # Convert the inputs from channels_first (NCHW) to channels_last (NHWC).
                inputs = tf.transpose(inputs, [0, 2, 3, 1])
            inputs = tf.identity(inputs, 'heatmaps')
        return inputs
